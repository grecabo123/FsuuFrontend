...
function Chairtman() {
    ...
    const StudentData = async () => {
        try {
            await axios.get('/api/chairman').then(res => {
                if (res.data.status === 200) {
                    setstudent(res.data.chairman);
                    setfilter(res.data.chairman);
                }
            })
        } catch (error) {
            console.log(error)
    ...
    const columns = [
        ...
        {
            name: "Action",
            cell: row => <Link to={`/admin/details/${row.id}`}><Badge value="Details" severity='info'></Badge></Link>,
            sortable: true,
        },
    ]
                ...
                <div className="container mt-3">
                    <div className="row">
                        <div className="col-md-12">
                            <DataTable
                                title="Chairman's Account"
                                noDataComponent={"No Chairman Account"}
                                columns={columns}
                                data={filter}
                                selectableRows
