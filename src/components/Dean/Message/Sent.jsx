...
function Sent() {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/fetchMessageSentFaculty/${id}`).then(res => {
    ...
    const column = [
        ...
        {
            name: "Action",
            cell: row =>  <Link to={`/faculty/view/refid=${row.message_id}`} className='border-0'><Badge value="Open" severity='info' /></Link>,
            sortable: true,
        }
    ];
