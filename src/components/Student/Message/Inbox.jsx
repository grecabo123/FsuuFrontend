...
function Inbox() {
    ...
    useEffect(() =>{
        ...
        axios.get(`/api/StudentInbox/${id}`).then(res =>{
            if(res.data.status === 200){
                setstudentInbox(res.data.StudentInbox);
            }
            setloading(false)
        }).catch((error) =>{
    ...
    const column = [
        {
            name: "From",
            selector: row => row.frominfo.email,
            sortable: true,
        },
        {
            name: "Subject",
        ...
        {
            name: "Action",
            selector: row => row.seen === 0 ? <button onClick={() => Update(row.id)} className='bg-transparent border-0'><Badge value={"New Message"} severity="danger"></Badge></button> : <Link to={`/student/read/ref=${row.id}`}><Badge value={"Read"} severity="info"></Badge></Link>,
            // to={`/dean/read/${row.id}`}
